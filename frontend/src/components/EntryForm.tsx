...
export default function EntryForm({ entry, defaultGroup, onSubmit, onClose }: Props) {
  const [title, setTitle] = useState(entry?.title ?? '')
  const [description, setDescription] = useState(entry?.description ?? '')
  const [severity, setSeverity] = useState(entry?.severity ?? 'minor')
  const [group, setGroup] = useState(entry?.group ?? defaultGroup ?? 'incoming_control')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      await onSubmit({ title, description, severity, group, assigned_to: assignedTo })
      onClose()
    } catch (err) {
